...
| `/thinking [off\|low\|medium\|high]` | how much reasoning to ask the provider for, and which field it goes in (default `off`: ask for none). The level belongs to the run: `/model`, `/provider`, `/reload` and `/fork` keep it, so picking another model does not quietly turn your preset off — and `/resume`/`/import` take the level the conversation they moved to recorded, which is the file having the last word |

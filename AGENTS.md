...
| `src/web.rs` | `--web`: the embedded viewer and the loopback listener that serves it, including `GET /file` — the route that reads a file a tool result named, for the page's preview panel (§12) — and `GET /jobs` with the `event: jobs` frame, the run's own background work for the header's jobs panel (§13). It also owns `export_html`, the other way that page leaves the process: the same `view.html` with a finished conversation welded into it as a JSON island (the session's own lines, `cwd` taken out, `<`/`>`/`&` escaped) — §14 of `docs/web-mode.md` is why there is one renderer rather than two |

...
|---|---|
| `src/main.rs` | argument parsing, the REPL, slash commands (including `/jobs` and `/jobs stop <pid>`, a person's door onto the same record the page and `job_op` read, `/import <file>`, which copies a conversation in from a file this run did not start from and leaves that file alone, and the two sends a command can be — `/skill <name>` and `/prompt <name>`, or a saved prompt typed as its own name — which is the one place a line typed at the prompt becomes something other than itself), one-shot `exec`, and the input reader the key thread and the hangup watcher belong to. `--no-session` is decided here and nowhere else: the flag's command-line refusals, and the one sentence every door that would open a conversation gets — `/new`, `/resume`, `/import`, and the page's sidebar rows, which go through the same dispatcher |
...
| `docs/web-mode.md` | the browser view over a running flint: all three levels are built (read-only view, live feed, composer with a sidebar), and §11, §12 and §13 are the measured record from a real browser — the page's controls, the file preview behind `GET /file`, and the jobs panel behind `GET /jobs` |

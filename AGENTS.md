...
| `src/web.rs` | `--web`: the embedded viewer and the loopback listener that serves it, including `GET /file` — the route that reads a file a tool result named, for the page's preview panel (`docs/web-mode.md` §12) |
...
| `scripts/` | Node replay tools: `vtscreen.js`, `term-layout-test.js`, `layout-trace.js`, and `browser-controls-test.js` (the one that needs a browser: it drives the page's own controls over the DevTools protocol and checks each press against the run's stdout, run by hand rather than in CI) |
...
| `docs/web-mode.md` | the browser view over a running flint: all three levels are built (read-only view, live feed, composer with a sidebar), and §11 and §12 are the measured record from a real browser — the page's controls, and the file preview behind `GET /file` |

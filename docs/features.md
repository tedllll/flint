...
| preview | a directory (`path`) | the path's name ends in a separator, so the panel asks `GET /dir` first; any other path asks `/file`, whose refusal says `X-Flint-Dir: 1` when it turned out to be a directory and the panel then reads the listing. There is no probe: being wrong costs one request (§27 of `docs/web-mode.md`) |
| preview | a listing | one `button.path` per entry, in the run's order: `..` first when there is a parent (a direction rather than an entry — dim, tooltipped `up to <dir>`), then the run's own lines — `sub dir/` for a directory, `notes.txt  (12 bytes)` for a file — with each row's tooltip the **full path the run built**, and the note counting what the route counted (`the first 2000 of 4321 entries` when one answer cannot carry the directory). Pressing a row reads that row's path: a directory goes in, a file opens beside the turn. The raw/rendered switch is put away, because there is no Markdown reading of a directory |
...
- **Driven in a real browser**: `scripts/browser-controls-test.js`, **113 claims** held, each checked

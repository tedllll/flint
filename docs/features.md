...
| sidebar | a row's `⋯` menu | that conversation's destructive rows, each written as what it does rather than as the line it sends (`delete one`, `file one away, out of the list` — the frame's own words), each behind the same two-press rule, and — on the current row only — a rename field whose submit reads `name this conversation` |

...
| `/resume <n\|id>` | switch to one | `resumed: <file> (N messages, model M)`, then the **transcript of that conversation is drawn**, then a note if it was a copy of another | selector (the sidebar's rows) | a bare `/resume` prints usage; `--no-session` refuses it; a fresh system prompt is rebuilt rather than the file's being trusted; the run is then held at that file's own last `thinking` and `schema` lines (the startup path's rule — the level and the shape travel with the conversation), not at the ones belonging to the conversation it left |
| `/fork [n]` | start a new conversation cut at question `n` | with no argument, the questions are listed so you can pick — each as `  <n>. <first line of the question>`; then `forked: <source> → <new file>` and `<N> messages kept, cut at question <n> of <M>: <that question's first line>` | selector: the page's dialog lists one labelled button per question, and the transcript puts **fork from here** under each answer a cut would keep | a turn boundary is the only place a conversation can be cut; a fold's summary is not a question (it is a `user` message so endpoints accept it, and listing it numbered every real question one higher than the transcript); cutting in front of the first question is refused only when the copy would be *empty*, so after `/compact` it keeps the summary and works; the branch is held at what the run is at, level and shape included, because its own file has neither yet |
| `/import <file>` | copy a conversation in from a session file | `imported: <file> (N messages) into <new file>`, then its transcript | form | the source is never written to; importing the file this run is writing is refused; a file with no messages is refused; the imported conversation's own `thinking` and `schema` lines are what the run is held at afterwards, exactly as `/resume`'s are |
...
| `GET /file?path=<p>[:<line>]` | `path` required, percent-encoded; resolved by `config::resolve_path` — a leading `~/`/`~\` is the home directory, an absolute path is used as written, anything else is against the run's `cwd`; the literal path is tried first, then the `:N`-stripped one | the file's bytes (`text/plain`, cut at 512 KB with `X-Flint-Cut`/`X-Flint-Size`), or the route's own refusal: a directory, a file over 64 MB, a non-UTF-8 file, or nothing there. The **directory** refusal carries `X-Flint-Dir: 1` beside its sentence, because the page acts on it — and what it does now is *open* the directory rather than list it |
| `GET /dir?path=<p>` | `path` required, percent-encoded; resolved exactly as `GET /file` resolves it; `~` included | the listing the panel draws, in the order the `list` tool prints it: `{"path","parent","entries":[{"name","line","path","dir","size"}],"total","shown"}`. Each entry's `path` is joined **by this process** (a name with a space is one name) and its `line` is the text the `list` tool would print for it (`name/`, or `name  (N bytes)`). At most **2000** entries, with `total`/`shown` saying so. Refusals: `404 nothing at <p>: …`, `400 <p> is a file, not a directory`, `400 cannot list <p>: …` when the directory cannot be read, and `400` for a missing or empty `path`. A press on a directory normally *opens* it (`POST /open`); this route is what the panel draws when that is refused because the run is `readonly` |
| `GET /resolve?text=<t>` | `text` required, percent-encoded, and it is the line **from the first character of the candidate path** | `{"path":"…","used":N}` — the longest prefix of that text which exists on this machine (`resolve_path`, so `~` and relative names are the run's own), and `N` the UTF-16 code units of it, which is what the page `slice`s the line at. Trailing punctuation is trimmed at each step and a trailing `:N`/`:N:C` is tried as a line number after the literal path, exactly as `GET /file` does. `404 nothing in <t> names a path this run can reach` — in a sentence, because it is a finding rather than a failure — and `400` for a missing or empty `text` |
...
| settings | any other row | sends the line the frame gives it — the same line the `/` menu would complete, and the same one a person could type. A row whose values carry **labels** (today only `/fork`, whose values are question numbers) puts the label where the help would repeat: one button per question, reading `/fork 2` and the question it cuts in front of |
...
| sidebar | a row's `⋯` menu | that conversation's destructive rows, each written as what it does rather than as the line it sends (`delete one`, `file one away, out of the list` — the frame's own words), each behind the same two-press rule, and — on the current row only — a rename field whose submit reads `name this conversation`. It is drawn against its row and opens upward when the row is at the bottom of a scrolled list, so it is never outside the box the sidebar paints in. It works on the row it was opened from, the run's own included: deleting that one starts a fresh conversation and removes the file, so the sidebar loses the row and the right side goes back to the page the viewer opens with, where the next thing said starts a conversation of its own |
...
| transcript | a `button.path` | opens the preview panel at that file, and at that line when the token named one. The button **reads what the token said** — `src/web.rs:412`, `src/web.rs#L412`, `C:/x.js:42` — so a `grep` hit keeps the line it is worth reading for; a `file://` prefix is dropped on the way, since the scheme is the one part of the token the panel does not need (§18 of `docs/web-mode.md`). A path that is a **directory** is **opened where it lives** — the same act as the panel's `open` control, `POST /open` — rather than previewed: a directory is a place, and a press on one is a request to go there (§28) |
| transcript | a line of the run's own listing | `list` prints one entry per line as `NAME/` or `NAME  (N bytes)`, and such a line is read as **one name**: `My Projects/` is a button reading `My Projects` with the run's `/` left as text, and `Application Data  (0 bytes)` one reading `Application Data`. It is the one place a name with a space can be known to end where it ends, and the name resolves against the run's working directory like any other relative path (§27 of `docs/web-mode.md`) |
| transcript | a **bare absolute path with a space in it**, in the model's own prose | the page cannot tell `C:\work\My Notes` from a path and a word, so it asks the run: `GET /resolve` with the line from the path's first character, and the answer's `used` (UTF-16 code units) is where the button ends. One button over the whole name, with the words after it still the sentence they were; the answer is cached per text and one request is made per text, and answers redraw only the blocks whose text asked. Only a token that *looks* absolute is asked about, and only when a word follows it; with no run behind the page (an export, a dropped session) nothing is asked and the first fragment stays the button (§28) |
...
| transcript | `fork from here`, under an answer | sends the frame's own `/fork <n>` line, cutting a new conversation in front of the question that answer belongs to: the branch keeps everything up to and including the answer, and the page is left reading it (`/new`'s kind of move, so the page re-reads the session). It is drawn **only where the page can point at the question a value names**: the frame's `/fork` row carries the questions' numbers (`values`) *and* their first lines (`labels`), the page pairs the two lists with its own turns **from the bottom** — where they agree even after `/compact`, which drops a prefix of the questions and never the newest — and believes a pairing only when that turn's own first line is the question the label names and is the only turn that reads that way. The first value is never a button (it cuts in front of everything the run still holds), a repeated question is skipped rather than guessed at, and while a turn runs every pairing is one question out of step and so none is drawn — the dialog's labelled list still names them all (§26 of `docs/web-mode.md`) |
| transcript | `thinking`, `instructions` | `<details>` blocks, both closed to begin with |
| preview | `reload`, `close` | re-reads the route that drew it — `GET /image` for a picture, `GET /file` for text, `GET /dir` for a listing; closing leaves the transcript and the reader's place alone, and lets go of the picture's blob URL |
| preview | a directory (`path`) | **a press on a directory opens it where it lives** (`POST /open`, §28): the hint says what happened and with what, and no panel is drawn at all. A path whose name ends in a separator is not even asked of `/file` first; any other path asks `/file`, and the `X-Flint-Dir: 1` header on its refusal is what turns that press into an open rather than a listing. The **one** case that still reads the directory is a `readonly` run: `POST /open` answers `409`, the panel shows the route's sentence and then `GET /dir`'s listing, so a reader who may not open it can still look. There is no probe: being wrong costs one request |
| preview | a listing | one `button.path` per entry, in the run's order: `..` first when there is a parent (a direction rather than an entry — dim, tooltipped `up to <dir>`), then the run's own lines — `sub dir/` for a directory, `notes.txt  (12 bytes)` for a file — with each row's tooltip the **full path the run built**, and the note counting what the route counted (`the first 2000 of 4321 entries` when one answer cannot carry the directory). Pressing a row asks for that row's path: a file opens beside the turn, and a directory is **opened where it lives** like any other press on one. The raw/rendered switch is put away, because there is no Markdown reading of a directory. A listing is only reachable in a `readonly` run, which is the case above |
...
- **`window.open(` is forbidden**: an address opens in a new tab as a link, and a path opens in this
  page's own panel — or, when it names a directory, where it lives. The one thing that leaves the page
  and starts a program is `POST /open`, and every route to it is a press the reader makes (§12.3, §28).
...
- **Driven in a real browser**: `scripts/browser-controls-test.js`, **118 claims** held, each checked
  against the run's own stdout — including the branch button: that no button is drawn while a single
  question leaves nothing to cut, that the button lands under the answer the cut keeps once a second
  question exists, that pressing it makes the run print `cut at question 2 of 2: …`, and that the page is
  then reading the branch rather than the conversation it was cut from. It prints what it drives before
  it presses anything, and its "not driven
  ...
  command lines are held by `src/web.rs`, and a press on a **directory that is gone** is what proves the
  routing with no window), a paste, an IME, a screen reader, two tabs, touch, a phone
...
- **Pure functions**: `scripts/web-view-test.js` (the splitter, the frames, the form composition, the
  ...
  a fence, a nested list and a pipe table are checked without a browser — `POST /open`'s body and guard,
  and the path-end resolution: one button over a name the run found, the `:N` inside it, and the refusal
...
- **No OS-level open without a press.** A file preview is the page's own panel (`GET /file` for text,
  ...
  deliberate second press in the panel's head (§12.3), a press on the picture itself (§21), or a press
  on a **directory**, which is what pressing a directory means (§28). Every one of them is a press the
  reader makes, refused outright in a `readonly` run, and none of them is the page deciding on its own.
  What the page still does not do is open the preview panel's *contents* anywhere: text there is plain
  text.

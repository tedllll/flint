...
| A switch alone adds no conversation to the sidebar | the same press, with the sidebar's rows read before it and after it | none. A run-level decision is written into the conversation's file once there is one and no longer creates it: eight doors (`--thinking`, `--schema`, `/thinking`, `/model`, `/provider`, `/reload`, `/config set` and these very rows) each left a file with no `chat` line in it, measured in a real home on 2026-09-23 — see `HANDOFF.md`. The browser harness was asserting this defect until the fix: it read the row the run is *writing*, which a switch used to make |
| The run's own row arrives with the first thing it says | `/name …` typed into the composer, then that row read | the row exists and is the `current` one, which is what the sidebar menu below belongs to — and the fixture rows belong to other conversations, which is why the menu on *them* offers no `/name` field |
